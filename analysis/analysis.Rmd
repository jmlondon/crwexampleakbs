...
abstract: >
  ...
  estimating the movement paths of animals from telemetry data. The package
  is specifically designed with marine mammals in mind, but other species
  in other habits with similar telemetry devices will work. This package
  compendium provides step-by-step instructions for importing and 
  setting up the telemetry data, initiating the `crwMLE()`, `crwPredict(),
  and `crwSimulator()`/'crwPostIS()` functions and visualizing results. The
  write up also discusses use of the `fix_path()` function to re-route
  predicted tracks around land. A few examples of data visualization
  plots are also presented. This compendium was originaly developed in 
  support of a training course at the NOAA Fisheries Protected Species
  Assessment Workshop in La Jolla, California, USA.
output: 
  uswebr::html_uswds:
...
separate tracks for each deployment. The code to derive tracks from point 
data within `sf` is a bit more complicated but is demonstrated below. It is
important that you include the `do_union = FALSE` argument to the
`dplyr::summarise()` function to maintain point order when creating the lines.
...
data. In this case, we are interested in Alaska and the Bering Sea.
...
And, now, we can plot our data again
...
We will use the ESRI Ocean Basemap for our background layer.
...
running `crwMLE`. The answer is, yes.
...
# predict hourly locations from fit movement model. But, make sure the
# previous fit process was successful before predicting.
tbl_data <- tbl_data %>% 
...
The first thing we need to do is convert the `crwPredict` results to 
`sf` objects. We can do that with a convenience function within the
latest (you'll need the devel branch from GitHub) version of `crawl`. Note
that we specify `locType = "p"`. By default, `crwPredict` returns predictions
at both the specified interval (in this case, hourly) and the time point for
each corresponding observed location. By specifying `locType = "p"` we only get
the predicted locations at the regular interval of `1 hour`.
...
unusual for the predicted tracks to cross land at times. We, ideally, would 
include a land mask as part of the crwMLE model fit. But, that becomes
computationally challenging. As a compromise, `crawl` includes a `fix_path()`
function that (*is very much in beta development*) will re-route portions of the predicted path around land.
...
Now, let's update our nested tibble with `*_fix` columns. We can also take a
...
```{r animate, warning=FALSE, message=FALSE, fig.width=6}
library(gganimate)
library(ggspatial)
...
pts_by_day <- tbl_data %>% 
  ...
  dplyr::mutate(yday = lubridate::yday(date_time)) %>% 
  sf::st_as_sf(crs = 3571) %>% 
